...
class Pagamento(models.Model):
    ...
    confirmado = models.BooleanField(default=True, help_text="Se marcado, gera movimento na tesouraria")
    ...
    def registrar_movimento_pagamento(self):
        if self.confirmado and not self.movimento_gerado and self.valor_pago > 0:
            ...
            Movimento.objects.create(
                ...
                evento=self.participante.evento if self.participante else self.evento,
                descricao=f"Pagamento inscrição - {self.participante.nome} ({self.participante.evento.nome})" if self.participante else f"Pagamento inscrição - {self.evento.nome}"
            )
    
            self.movimento_gerado = True
            self.save(update_fields=['movimento_gerado', 'conta'])
